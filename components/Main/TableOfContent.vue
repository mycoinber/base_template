    ...
    <div class="container">
      <nav class="w-full p-4 rounded-[0.625rem] bg-background-02" style="border: 1px solid var(--border);">
        <div class="flex items-center justify-between cursor-pointer select-none" @click="toggle">
          <span class="text-2xl font-font-02 uppercase max-[541px]:text-xl">{{ $t('table_of_content') }}</span>
        ...
        <div :class="['overflow-hidden transition-all duration-300', { 'max-h-[25rem] opacity-100': isOpen }, { 'max-h-0 opacity-0': !isOpen }]">
          <ul class="flex flex-col gap-2 list-none pt-4 m-0" itemscope itemtype="https://schema.org/ItemList">
            <li v-for="(item, index) in data.article.blocks" :key="item._id" class="text-color-white relative pl-8 transition-all duration-300 opacity-50 text-sm m-0 hover:text-color-01 hover:opacity-100"
