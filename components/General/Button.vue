...
<template>
    <button v-if="isOfferAction" type="button" class="font-font-02 text-base font-medium text-color-white text-center uppercase w-full min-w-0 px-6 py-4 rounded-[0.4rem] relative z-10 overflow-hidden before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-full before:bg-color-01 before:z-[-1] before:transition-[filter] before:duration-300 hover:before:brightness-[0.7]" @click="openOffer(null, data.target || '_blank')">
        {{ data.title }}
    </button>
