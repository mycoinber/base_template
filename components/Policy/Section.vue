...
<template>
  <section :id="data.key" class="my-8 max-[541px]:my-4">
    <div class="container">
      <div class="flex flex-nowrap gap-8 w-full max-[541px]:flex-col">
        <div v-if="data.type === 'section'" v-html="contentHtml" class="flex-1 [&_a]:text-color-01"></div>

        <div v-if="data.images?.length" class="flex-1 rounded-[0.625rem] overflow-hidden">
          <img :src="`/media${data.images[0]?.path || ''}`" :alt="data.images[0]?.alt || 'image'" class="w-full h-full object-cover" />
        </div>
      </div>
    </div>
  </section>
